...
overview: 构建一个三栏布局的 JSON 元数据解析与 JSONPath 抽取工具。左侧输入 JSON 并格式化显示，中间自动解析所有叶子节点路径并支持多选（含中英文翻译按钮），右侧输出 JSONPath 映射格式，数组字段支持查看内容后选择索引。
design:
  architecture:
    framework: html
  styleKeywords:
...
todos:
  - id: create-html-structure
    content: 使用 [skill:UI/UX Design and Development] 搭建三栏布局的 HTML 骨架，包含顶部标题栏、左侧 JSON 输入区、中间键名选择区、右侧结果输出区
    status: pending
...
- **搜索筛选**：顶部搜索框支持按英文键名或中文翻译实时过滤。**搜索触发时自动取消当前预览状态**——若搜索导致当前预览按钮被隐藏，预览状态、左侧高亮、滚动定位均重置为默认，避免"预览一个看不见的按钮"的困惑
- **批量操作**：搜索框旁边提供 `全选`（选中当前可见字段）、`反选`（反转当前可见字段选中状态）、`清空`（取消所有选中）三个快捷按钮，方便大量字段的操作效率
- **翻译机制**（DeepSeek API 批量翻译 + 内置词典兜底）：
- 页面顶部提供 **API Key 输入框**，用户自行填入 DeepSeek API Key（仅存储在浏览器内存中，不持久化、不上传）
- 格式化 JSON 后，收集所有叶子节点的 `keyName` 去重列表，**一次性批量调用** DeepSeek API 获取中文翻译（提交 prompt："将以下英文字段名翻译为中文，返回 JSON 格式 { field: translation }"）
...
- 页面底部展示完整的翻译对照表（去重：同一 keyName 只显示一条，不因多条路径重复），标注翻译来源（词典/API/本地）
- **左侧高亮联动**：点击中间面板的键名按钮时，左侧 JSON 树中对应的节点自动展开父级路径、滚动到可视区域，并以闪烁高亮动画标记位置，帮助用户快速定位原始数据中的字段
- **同名键去重**：当多个叶子节点有相同键名时（如 `data.aweme_detail.duration` 和 `data.aweme_detail.video.duration`），自动在输出 key 前加上父级键名前缀进行区分（如 `duration` 和 `video_duration`）。去重使用 `outputKey` 而非 `keyName` 检测冲突（检测最终输出的 key 是否重复）。每个条目有 `userEdited: boolean` 标记，用户手动编辑过的 outputKey 不会被自动覆盖。去重执行时机：任何选中状态变化或手动编辑后触发，收集所有 `state === 'selected'` 的 LeafPathEntry → 检查 `outputKey` 冲突 → 对冲突条目中 `userEdited === false` 的自动设置 `outputKey = 父前缀_keyName` → 如果编辑过的条目也与自动条目冲突，提示警告但不强改。**去重优先级**：路径更短的条目保留原始 outputKey，路径更长的加父级前缀（确保稳定性）
- **数组处理**：当叶子路径经过数组节点时，按钮显示为特殊样式（橙色边框 + `⟳` 图标提示需要选择下标），点击后弹出对话框：
...
- **下标记忆**：用户选择下标后，按钮进入"预览中"状态。若用户取消选中后重新回到该按钮，之前选的下标保留，无需重选。已选下标的按钮旁显示 ✎ 小图标，点击该图标可重新打开下标选择弹窗修改选择
...
```
① 搜索：在中间面板搜索框输入中文关键词（如"播放"、"点赞"）
   → 筛选出中文翻译匹配的按钮

② 预览：点击某个按钮（如 play_addr_h264--H264播放地址）
   → 不立即选中，而是先触发左侧高亮联动
   → 左侧 JSON 树自动展开、滚动、闪烁高亮对应节点
   → 用户查看该字段的真实值和上下文

③ 确认：用户比对左侧高亮内容
   → "这个 url_list 里确实有视频地址，是我要的" → 确认
   → "不对，这个 duration 是视频时长不是音乐时长" → 换下一个

④ 选择：确认无误后，再次点击同一按钮 → 选中
   → 按钮变蓝高亮
   → 右侧自动追加 JSONPath 映射
   → 选错可以再点取消
```
...
- field 的 key **保持元数据原始键名**（即叶子节点的 keyName，不做驼峰转换，如 `aweme_id` 输出就是 `aweme_id`），同名键自动加父级前缀区分（如上例中两个 `url_list` → `url_list` 和 `cover_url_list`），value 为完整 JSONPath 表达式
- 每个字段旁提供**可编辑输入框**，允许用户手动修改输出的 key 名称。右侧面板以列表形式展示：每行 `[可编辑输入框(key)] → [只读文本(JSONPath值)]`，列表下方显示生成的 JSON 代码块（实时更新）
...
- **依赖**：DeepSeek API（翻译能力，用户自备 Key；API 不可用时自动回退本地翻译），其余零外部依赖，所有核心逻辑在浏览器端完成
...
1. **树形结构渲染**：采用递归 DOM 生成 + CSS 缩进方案，每个节点包含展开/折叠箭头、键名、类型标记和值。每个叶子节点 DOM 元素设置 `data-path` 属性存储其完整 JSONPath（如 `data-path="$.data.aweme_detail.aweme_id"`），用于高亮联动时的定位。与 JSONViewer 等库的方案一致，但完全自主实现以保持零依赖。

2. **路径解析策略**：深度优先遍历 JSON 树，遇到叶子节点（非 object、非 array 的值）时记录完整路径。遇到数组节点时，标记该路径需要用户选择下标，暂停该分支的继续遍历，等用户选择后再完成。**嵌套数组**（如 `bit_rate[0].play_addr.url_list[1]` 同时经过两层数组）需要依次弹窗选择每层下标，直到所有数组下标都确定后路径才算完整。
...
4. **数组下标选择交互**：当路径经过数组时（如 `video.bit_rate`），提取数组中每个元素的一级子字段摘要展示在弹窗表格中，用户可直观浏览内容并选择下标索引。

5. **左侧高亮联动**：点击中间键名按钮时，先自动展开该节点的所有父级折叠节点，然后使用**双重 `requestAnimationFrame`**（展开 → rAF → 再 rAF → scrollIntoView）确保浏览器完成 reflow 后再滚动，避免位置偏移。调用 `scrollIntoView({ behavior: 'smooth', block: 'center' })` 滚动到目标节点，最后添加 CSS 闪烁高亮动画（黄色脉冲背景 + 过渡效果），并在 2 秒后自动移除。

6. **同名键去重**：使用 `outputKey` 检测冲突（而非 `keyName`）。用户手动编辑过的条目设置 `userEdited = true`，自动去重时不会覆盖已编辑的 key。冲突的自动条目以倒数第二级父节点键名作为前缀（如 `duration` → `video_duration`），多级冲突则继续向上追加。去重仅对 `state === 'selected'` 的条目执行。

7. **JSON 校验错误处理**：使用 `try/catch` 包裹 `JSON.parse`，解析失败时捕获错误对象，提取行列信息展示在输入框下方，同时红色边框高亮输入框。只有校验通过后才执行后续的树构建和路径提取。

### 性能考量

- 对于用户提供的示例级 JSON（约 200KB），递归遍历和 DOM 渲染在 100ms 内完成
...
interface TreeNode {
  key: string;           // 当前层级键名
  path: string;          // 从根到当前节点的完整 JSONPath 路径（用于 DOM 的 data-path 属性关联）
  type: 'object' | 'array' | 'string' | 'number' | 'boolean' | 'null';  // 节点类型
...
interface LeafPathEntry {
  id: string;                // 唯一标识，基于不含数组下标的基础路径生成（如"$.data.video.bit_rate._.play_addr.url_list"），下标变化不影响 id，确保条目可跨状态追踪
  keyName: string;           // 叶子节点键名（如 "nickname"）
  fullJsonPath: string;      // 完整 JSONPath（如 "$.data.aweme_detail.author.nickname"）
  chineseName: string;       // 中文翻译（如 "作者昵称"）
  outputKey: string;         // 去重后的输出 key（默认为 keyName，重复时自动加父级前缀，用户可编辑修改）
  userEdited: boolean;       // 用户是否手动编辑过 outputKey（编辑过的 key 不会被自动去重覆盖）
  state: 'default' | 'previewed' | 'selected';  // 三态：默认→预览中→已选中
  arrayIndexes: Map<string, number>;  // 路径中包含的数组下标映射。Key为数组节点的完整路径（如"$.data.video.bit_rate"），避免不同路径同名数组冲突。Value为选中的下标
...
```javascript
// 注意：字典 key 不可重复。JS 对象字面量中重复 key 只会保留最后一个。
// duration/collect_count 等在不同模块下有相同 keyName 时，字典只保留一条（最通用的翻译）。
// 上下文区分由去重机制通过 outputKey 加父级前缀来完成（如 video_duration）。
const TRANSLATION_DICT = {
  ...
  duration: '时长',  // 多模块共用（aweme_detail/video/music 下均有 duration）
...
- 按钮采用 tag 风格，圆角矩形：
- 默认状态：浅灰边框，浅灰背景
- 预览中状态：蓝色虚线边框 + 轻微脉冲闪烁动画
- 已选中状态：主题色填充 + 白色文字
- 数组待选状态：橙色边框 + `⟳` 图标
- 数组为空状态：置灰不可点击
- 按钮布局采用 flex-wrap 流式排列，紧凑且易扫视
- 底部固定翻译对照表区域，表格形式展示，奇数行浅底色

### 右侧面板 — 结果输出区

- JSONPath 结果以代码块展示（深色底 + 语法高亮）
- 「一键复制」按钮位于代码块右上角，复制成功后显示绿色对勾动画反馈
- 统计信息显示已选字段数量

### 标题栏

- 左侧应用图标 + 名称「抖音 JSON 路径抽取器」
- 右侧 **DeepSeek API Key 输入框**（password 类型隐藏显示，仅存内存）+ 版本号和简单帮助提示

### 配色系统

- 主题色：蔚蓝色 #3B82F6，代表科技与数据
- 成功反馈：翠绿色 #10B981
- 深色背景：石板灰 #1E293B
- 面板背景：浅灰 #F8FAFC 与白色卡片层叠
